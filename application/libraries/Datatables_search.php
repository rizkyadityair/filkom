    ...
    */
    private function get_filtering()
    {
      ...
      for($i = 0; $i < count($mColArray); $i++)
        if($this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value'])) != '')
          if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
            if($this->check_cType())
              $sWhere .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value'])) . "%' AND ";
            else
              $sWhere .= $this->select[$this->columns[$i]] . " LIKE '%" . $this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value'])) . "%' AND ";
