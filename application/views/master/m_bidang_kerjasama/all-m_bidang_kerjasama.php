 ...
 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">M Bidang Kerjasama</li>
      </ol>
    </section>
    <!-- Main content -->
              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="pull-right">          
                    <button type="button" onclick="create()" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah M Bidang Kerjasama</button>
                    <!-- <a href="<?= base_url('fitur/ekspor/m_bidang_kerjasama') ?>" target="_blank">
                      <button type="button" class="btn btn-sm btn-warning"><i class="fa fa-file-excel-o"></i> Ekspor M Bidang Kerjasama</button> 
                    </a>
                    <button type="button" class="btn btn-sm btn-info" onclick="$('#modal-impor').modal()"><i class="fa fa-file-excel-o"></i> Import M Bidang Kerjasama</button> -->
                  </div>
                </div>  
              </div>
            </div>
  ...
  <div class="modal fade bd-example-modal-sm" tabindex="-1" m_bidang_kerjasama="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" id="modal-form">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Impor M Bidang Kerjasama</h4>
        </div>
        <form action="<?= base_url('fitur/impor/m_bidang_kerjasama') ?>" method="POST"  enctype="multipart/form-data">
        <div class="modal-body">
  ...
  <script type="text/javascript">
    ...
    function loadtable(status) {
          var table = '<table class="table table-condensed table-striped datatables" id="mytable">'+
             ...
             $("#load-table").html(table)
              var t = $("#mytable").DataTable({
                ...
                ajax: {"url": "<?= base_url('master/M_bidang_kerjasama/json?status=') ?>"+status, "type": "POST"},
                columns: [
                    {"data": "mbk_id","orderable": false, "className": "text-center"},
                    {"data": "mbk_id","orderable": false},
                    {"data": "mbk_name"},
                    ...
                    { 
                      targets : [0],
                        render : function (data, type, row, meta) {
                          ...
                          cb = cbinput.split(',');
                          var checked = "";
                          if(cb.includes(row['mbk_id'])) checked = "checked";
                          if(cbinput=="all") checked = "checked";
                          return "<input type='checkbox' onclick='checkdata($(this),"+row['mbk_id']+")' value='"+row['mbk_id']+"' "+checked+">";
                          }
                    },
                    ...
                    { targets : [3],
                        render : function (data, type, row, meta) {
                                if(row['status']=='ENABLE'){
                                  var htmls = '<a href="<?= base_url('master/M_bidang_kerjasama/status/') ?>'+row['mbk_id']+'/DISABLE">'+
                                            '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>'+
                                            '</a>';
                              }else{
                                  var htmls = '<a href="<?= base_url('master/M_bidang_kerjasama/status/') ?>'+row['mbk_id']+'/ENABLE">'+
      ...
      function edit(id,e) {
        ...
        $("#modal-form").modal();
        $("#title-form").html('Edit M Bidang Kerjasama');
        $("#load-form").load("<?= base_url('master/M_bidang_kerjasama/edit/') ?>"+id);
      }

      function create() {
        $("#load-form").html('loading...');
        $("#modal-form").modal();
        $("#title-form").html('Create M Bidang Kerjasama');
        $("#load-form").load("<?= base_url('master/M_bidang_kerjasama/create/') ?>");
      }
        ...
        Swal.fire({
          ...
          confirmButtonText: 'Yes, delete it!'
            ...
            if (result.isConfirmed) {
              $.ajax({
                  url: '<?= base_url('master/m_bidang_kerjasama/delete/')?>',
                type: 'post',
                dataType: 'html',
                data: {id: id},
                beforeSend:function () { },
      ...
      function hapuspilihdata() {
        ...
        confirmButtonText: 'Yes, delete it!'
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?= base_url('master/m_bidang_kerjasama/Deletedata/')?>',
              type: 'post',
              dataType: 'html',
              data: {data: data},
              beforeSend:function () {
